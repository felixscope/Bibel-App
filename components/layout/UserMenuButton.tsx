...
export function UserMenuButton() {
  ...
  // Not logged in - show person icon on mobile, "Anmelden" text on desktop
  if (!user) {
    return (
      <Link
        href="/auth/login"
        className="p-2.5 rounded-lg hover:bg-[var(--bg-hover)] transition-colors flex items-center"
        title="Anmelden"
      >
        <svg className="w-5 h-5 text-[var(--text-secondary)] md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
        </svg>
        <span className="hidden md:inline text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--accent)] transition-colors">Anmelden</span>
      </Link>
    );
  }
            ...
            <div className="p-2">
              ...
              {mounted && (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-colors"
                  role="menuitem"
                >
                  {isDark ? (
                    <>
