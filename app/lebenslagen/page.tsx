    ...
    <MainLayout>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        <motion.h1
          ...
          className="chapter-title text-3xl text-[var(--text-primary)] mb-3"
        >
          Lebenslagen
        </motion.h1>
        <motion.p
          ...
          {LEBENSLAGEN.map((item, index) => (
            ...
            >
              <Link
                ...
                className="group flex items-start gap-4 rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] shadow-sm p-5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md hover:border-[var(--accent)]/30"
              >
                <div className={`w-10 h-10 rounded-xl bg-[var(--bg-secondary)] flex items-center justify-center flex-shrink-0 ${ICON_COLOR_MAP[item.icon] || "text-[var(--accent)]"}`}>
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-[var(--accent)] text-sm font-medium">
                    {item.passages.length} Bibelstellen →
                  </span>
                </div>
              </Link>
            </motion.div>
